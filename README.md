...
* **Polyglot & Historical Analysis:** 🌍 We support over 50 languages out-of-the-box using `tree-sitter`. Our graph is version-aware, allowing you to query the state of your codebase at any point in its Git history and perform powerful semantic diffs between commits.

* **Objective-Driven Context:** 🔗 Ragnaroc is designed to understand the "why" behind your code. By integrating with task trackers and parsing commit messages, we link code directly to the stories, tasks, and business objectives that prompted its creation.
...
graph LR
    subgraph "Phase 1: Semantic Search"
        direction LR
        A[User Query 💬] --> B(HyDE: Rewrite Query for Search 🔮);
        B --> C{Vector Search on Code Snippets 🧠};
        C --> D[Top-K Candidate Nodes 🎯];
    end
